...
export default function GlobalSearch({ open, onOpenChange }: GlobalSearchProps) {
 ...
 const RESULT_TYPES = {
 task: { label: t('globalSearch.tasks'), icon: CheckSquare, color: 'bg-[rgba(125,143,168,0.12)]0' },
 content: { label: t('globalSearch.content'), icon: FileText, color: 'bg-naya-olive-060' },
 lead: { label: t('globalSearch.leads'), icon: Users, color: 'bg-[rgba(158,126,135,0.12)]0' },
 strategy: { label: t('globalSearch.strategy'), icon: BarChart3, color: 'bg-[rgba(212,201,122,0.12)]0' },
 };
 ...
 <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-naya-olive-35" />
 <Input
 ...
 <Search className="h-12 w-12 text-naya-olive-35 mx-auto mb-4" />
 <h3 className="text-lg text-foreground mb-2">
 {t('globalSearch.searchEverything')}
 </h3>
 <p className="text-naya-olive-55 ">
 ...
 <div
 key={result.id}
 className="p-3 rounded-lg border hover:bg-naya-olive-06 :bg-naya-olive cursor-pointer transition-colors"
 onClick={() => handleResultClick(result)}
 data-testid={`search-result-${result.type}`}
 >
 <div className="flex items-start gap-3">
 <result.icon className="h-4 w-4 text-naya-olive-35 mt-1 flex-shrink-0" />
 <div className="flex-1 min-w-0">
 <h5 className="font-medium text-foreground truncate">
 {result.title}
 </h5>
 <p className="text-sm text-naya-olive-55 mt-1 line-clamp-2">
 {result.description}
 </p>
