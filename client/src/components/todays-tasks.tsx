...
const ENERGY_BADGES: Record<string, { icon: React.ElementType; label: string; cls: string }> = {
 deep_work: { icon: Target, label: 'Focus', cls: 'bg-naya-olive-06 text-[#354963] ' },
 creative: { icon: Sparkles, label: 'Créatif', cls: 'bg-[rgba(158,126,135,0.12)] text-[#5c3d45] ' },
 admin: { icon: ClipboardList, label: 'Admin', cls: 'bg-naya-olive-10 text-naya-cream0 ' },
 social: { icon: MessageCircle, label: 'Social', cls: 'bg-naya-olive-06 text-naya-olive ' },
 logistics: { icon: Package, label: 'Logistique', cls: 'bg-[rgba(212,201,122,0.12)] text-[#5a4f0d] ' },
 execution: { icon: Zap, label: 'Action', cls: 'bg-[rgba(212,201,122,0.12)] text-[#5a4f0d] ' },
};
...
function PlannerTaskPopover({ task, projects, onToggle, isToggling }: {
 ...
 {task.description && (
 <p className="text-xs text-naya-cream0 mt-1 line-clamp-3">{task.description}</p>
 )}
 </div>
 </div>
 ...
 {task.category && (
 <Badge className={`${CATEGORY_COLORS[task.category] || 'bg-naya-olive-10 text-naya-olive-55'} text-xs h-5 border-0`}>
 {task.category}
 </Badge>
 )}
 {task.estimatedDuration && (
 <span className="text-xs text-naya-olive-35 flex items-center gap-0.5">
 ...
 <div className="bg-naya-olive-06 border border-naya-olive-10 rounded-md px-2.5 py-2">
 <p className="text-[11px] text-[#354963] leading-snug">
...
export default function TodaysTasks() {
 ...
 <div key={task.id} className="flex items-start gap-3 p-4 rounded-lg border border-border/60 bg-muted/30">
 ...
 <p className="text-naya-olive-35 text-sm">Aucune tâche planifiée pour aujourd'hui</p>
 <button
 onClick={() => generateTasksMutation.mutate()}
 disabled={isGenerating}
 className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold shadow hover:bg-primary/90 transition-all disabled:opacity-60"
 >
 {isGenerating ? (
 ...
 <p className="text-xs text-naya-olive-35 ">Naya analyse tes projets et objectifs actifs</p>
 </div>
 )
 ) : viewMode === 'list' ? (
 <div className="space-y-3">
 ...
 className="flex items-start gap-3 p-3.5 rounded-lg bg-muted/50 border border-border opacity-60"
 >
 <Checkbox
 checked={true}
 ...
 {group && (
 <div className="flex items-center gap-2 py-1.5 border-b border-dashed border-naya-olive-18 mb-1">
 <span className="text-xs text-naya-cream0 uppercase tracking-wide">
 ▸ {group}
 </span>
 {groupTotalMin > 0 && (
 <span className="text-xs text-naya-olive-35 ">· ~{groupTotalMin}m</span>
 )}
 <span className="text-xs text-naya-olive-35 ">· {groupTasks.length} {t('todaysTasks.tasksLabel')}</span>
 </div>
 )}
 ...
 key={task.id}
 data-task-title={task.title}
 className={`flex items-start gap-3 p-4 rounded-lg border transition-all duration-150 hover-lift ${group ? 'ml-2' : ''} ${
 ...
 {isBlocked && !isMilestoneBlocked && <Lock className="h-3 w-3 flex-shrink-0 text-naya-sulphur" />}
 <h3
 ...
 <p className="text-xs text-[#5a4f0d] mt-0.5 flex items-center gap-1">
 ...
 <div
 key={h}
 className="absolute w-full border-t border-naya-olive-10 flex items-start"
 style={{ top: `${(h - 7) * 64}px`, height: 64 }}
 >
 <span className="text-xs text-naya-olive-35 w-10 flex-shrink-0 -mt-2.5 pr-2 text-right">
 {formatHour(h)}
 </span>
 <div className="flex-1 ml-2 border-l border-naya-olive-10 h-full" />
 </div>
 ))}
 ...
 {task.estimatedDuration && (
 <span className="text-[10px] text-naya-olive-35 ">~{task.estimatedDuration}m</span>
 )}
 </div>
 </div>
 </PopoverTrigger>
 ...
 <blockquote className="border-l-2 border-primary/40 pl-3 text-sm text-naya-olive-55 italic">
 {replanPreview.reasoning}
 </blockquote>
 )}
 ...
 <div key={i} className="flex items-start gap-2 p-2.5 border border-naya-olive-10 rounded-lg">
 <TypeIcon type={t.type} className="h-4 w-4 flex-shrink-0 text-muted-foreground mt-0.5" />
 <div className="min-w-0">
 <p className="text-sm text-foreground ">{t.title}</p>
 {t.category && (
 <span className={`text-xs px-1.5 py-0.5 rounded-full ${CATEGORY_COLORS[t.category] || 'bg-naya-olive-10 text-naya-olive-55'}`}>
 {t.category}
 </span>
 )}
 </div>
