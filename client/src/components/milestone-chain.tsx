...
const STATUS_CFG: Record<string, { icon: React.ReactNode; label: string; dot: string; row: string; text: string }> = {
 ...
 unlocked: { icon: <Unlock className="h-3 w-3" />, label: "Débloqué", dot: "bg-[rgba(125,143,168,0.20)] text-[#354963] ", row: "", text: "text-foreground" },
 active: { icon: <Zap className="h-3 w-3" />, label: "Actif", dot: "bg-primary/10 text-primary", row: "", text: "text-foreground font-medium" },
 completed: { icon: <Check className="h-3 w-3" />, label: "Complété", dot: "bg-naya-olive-10 text-naya-olive ", row: "opacity-70", text: "text-muted-foreground line-through" },
...
export default function MilestoneChain({ projectId, projectName }: MilestoneChainProps) {
 ...
 milestone.status === "completed" ? "bg-naya-olive-18 " :
 milestone.status === "locked" ? "bg-border" :
 "bg-primary/20"
 }`} />
 )}

 <div className={`flex items-center gap-2.5 rounded-lg px-3 py-2 transition-opacity ${cfg.row}`}>
 {/* Dot statut */}
